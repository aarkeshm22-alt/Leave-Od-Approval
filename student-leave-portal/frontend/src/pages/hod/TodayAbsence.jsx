...
const TodayAbsence = () => {
  ...
  const getStudentField = (item, field, fallback = 'N/A') => {
    if (item.student && item.student[field] !== undefined && item.student[field] !== null) {
      return item.student[field];
    }
  ...
  // 🔥 Helper to determine request type (Leave or On-Duty)
  const getRequestType = (item) => {
    const type = item.type || item.class || '';
    if (type.toLowerCase().includes('leave')) return 'Leave';
    if (type.toLowerCase().includes('on-duty') || type.toLowerCase().includes('od')) return 'On-Duty';
    return 'On-Duty'; // default
  };
  ...
  const fetchData = async () => {
    try {
      ...
      const [leavesRes, odRes] = await Promise.all([
        fetch(`${BASE_URL}/api/leaves/hod/pending?tab=ACTIONED`, { headers }),
        fetch(`${BASE_URL}/api/od/hod/pending?tab=ACTIONED`, { headers })
      ]);
      ...
      const odList = odData?.data || [];

      const allRequests = [...leavesList, ...odList];
      ...
      const mapped = todayAbsences.map(item => {
        const student = item.student || {};
        const rawSection = student.section || item.section || 'N/A';
        const type = getRequestType(item);
        return {
          id: item._id || item.id,
          year: student.year || item.year || 'N/A',
          section: rawSection,
          sectionLetter: getSectionLetter(rawSection),
          registerNo: student.registerNo || item.registerNo || 'N/A',
      ...
      console.error('Failed to fetch today\'s absence data:', error);
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      writeFile(workbook, `Today_Absence_${new Date().toISOString().slice(0,10)}.xlsx`);
    } catch (err) {
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      doc.text("Today's Absence Report", 14, 13);
      ...
      doc.save(`Today_Absence_${new Date().toISOString().slice(0,10)}.pdf`);
    } catch (err) {
  ...
  if (errorMsg) {
    return (
      <div className="p-4 sm:p-6 bg-rose-50 border border-rose-200 text-rose-800 text-xs font-bold rounded-xl flex items-center gap-2">
        ...
        <button
          onClick={fetchData}
          className="ml-auto px-3 py-1 bg-rose-100 hover:bg-rose-200 rounded-lg text-rose-700 transition-colors"
        >
          Retry
        </button>
      </div>
  ...
  // Split data – using the new `type` field which is now reliable
      ...
      <div className="border-b border-slate-200/60 pb-4 sm:pb-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 font-medium mt-1">
            Students on approved Leave or On‑Duty Today{' '}
            <span className="font-bold text-amber-600 underline">({new Date().toLocaleDateString()})</span>
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-2 text-xs text-slate-500 bg-slate-50 border border-slate-200 px-3 sm:px-4 py-2 rounded-full">
            ...
            <span className="font-medium">
              {totalAbsences} {totalAbsences === 1 ? 'student' : 'students'}
            </span>
          </div>
          <button
            onClick={fetchData}
            className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-2 bg-white border border-slate-300 rounded-full hover:bg-slate-50 transition-colors shadow-sm"
          >
          ...
          <button
            ...
            disabled={isExporting || absenceData.length === 0}
            className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-2 bg-green-50 border border-green-200 text-green-700 rounded-full hover:bg-green-100 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            disabled={isExporting || absenceData.length === 0}
            className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-2 bg-red-50 border border-red-200 text-red-700 rounded-full hover:bg-red-100 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Download size={14} />
            <span>PDF</span>
          </button>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[700px] text-left text-[10px] sm:text-xs divide-y divide-slate-200">
                <thead className="bg-slate-50 text-slate-500 text-[8px] sm:text-[10px] uppercase font-bold tracking-wider">
                  <tr>
                    <th className="p-2 sm:p-4 pl-3 sm:pl-6">Year</th>
                    <th className="p-2 sm:p-4">Section</th>
                    <th className="p-2 sm:p-4">Reg No</th>
                    <th className="p-2 sm:p-4">Student Name</th>
                    <th className="p-2 sm:p-4">Reason</th>
                    <th className="p-2 sm:p-4">From Date</th>
